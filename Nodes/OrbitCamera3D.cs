...
public partial class OrbitCamera3D : Resource
{
  [Export] float SCROLL_SPEED = 10; // Speed when use scroll mouse
  [Export] float ZOOM_SPEED = 5; // Speed use when is_zoom_in or is_zoom_out is true
  [Export] float DEFAULT_DISTANCE = 20; // Default distance of the Node
  [Export] float ROTATE_SPEED = 10;
  [Export] NodePath ANCHOR_NODE_PATH;
  [Export] float MOUSE_ZOOM_SPEED = 10; //
  #region Event var
  Vector2 _move_speed;
  float _scroll_speed;
  Godot.Collections.Dictionary _touches;
  float _old_touche_dist;
  // Use to add posibility to updated zoom with external script
  bool is_zoom_in;
  bool is_zoom_out;
  #endregion

  #region Transform var
  Vector3 _rotation;
  float _distance;
  Node3D _anchor_node;
  #endregion
